...
def init():
    ...
    goal_x = 60 # the goal to reach is at the upper left of the map (the x-coordinate is 20 and not 0 because the car gets bad reward if it touches the wall)
    goal_y = largeur - 100 # the goal to reach is at the upper left of the map (y-coordinate)
...
class Car(Widget):
    ...
    signal1 = NumericProperty(0) # initializing the signal received by sensor 1 gives density of sand around signal 1
    ...
    #longurer is x axis and larguerer is y axis
    def move(self, rotation):
        ...
        self.sensor1 = Vector(30, 0).rotate(self.angle) + self.pos # updating the position of sensor 1, 30 is distance between car and sensor 1
...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        #-orientation ensures that the car explores both the directions more efficiently
        ...
        if sand[int(self.car.x),int(self.car.y)] > 0: # if the car is on the sand
            ...
            last_reward = -1 # and it gets bad reward (-0.2)
